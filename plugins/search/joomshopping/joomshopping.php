    ...
    function onContentSearch( $text, $phrase='', $ordering='', $areas=null )
    {
        ...
        switch ($phrase) {
            case 'exact':
                ...
                $wheres2     = array();
                $wheres2[]     = "prod.`".$lang->get('name')."` LIKE ".$text." OR man.`".$lang->get('name')."` LIKE ".$text;
                $wheres2[]     = "prod.`".$lang->get('short_description')."` LIKE ".$text." OR man.`".$lang->get('short_description')."` LIKE ".$text;
                $wheres2[]     = "prod.`".$lang->get('description')."` LIKE ".$text." OR man.`".$lang->get('description')."` LIKE ".$text;
                $wheres2[]     = "prod.product_ean LIKE ".$text;
                $where         = '(' . implode( ') OR (', $wheres2 ) . ')';
                $attr_where    = "attr.ean LIKE ".$text;
            break;
            ...
            default:
                ...
                foreach ($words as $word) {
                    ...
                    $wheres2     = array();
                    $attr_wheres2     = array();
                    $wheres2[]     = "prod.`".$lang->get('name')."` LIKE ".$word." OR man.`".$lang->get('name')."` LIKE ".$word;
                    $wheres2[]     = "prod.`".$lang->get('short_description')."` LIKE ".$word." OR man.`".$lang->get('short_description')."` LIKE ".$word;
                    $wheres2[]     = "prod.`".$lang->get('description')."` LIKE ".$word." OR man.`".$lang->get('description')."` LIKE ".$word;
                    $wheres2[]     = "prod.product_ean LIKE ".$word;
                    $wheres[]      = implode( ' OR ', $wheres2 );
                    $attr_wheres2[]= "attr.ean LIKE ".$word;
                    $attr_wheres[] = implode( ' OR ', $attr_wheres2 );
                }
                $where = '(' . implode( ($phrase == 'all' ? ') AND (' : ') OR ('), $wheres ) . ')';
                $attr_where = '(' . implode( ($phrase == 'all' ? ') AND (' : ') OR ('), $attr_wheres ) . ')';
            break;
        }
		
        $query = "SELECT DISTINCT attr.product_id FROM `#__jshopping_products_attr` as attr WHERE ($attr_where)";
        $db->setQuery($query);
        $rows = $db->loadResultArray();
		if (count($rows)) {
			$where .= ' OR prod.product_id IN ('.implode(',',$rows).')';
		}
