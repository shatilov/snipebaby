...
if (!empty($this->file[0])) {
	$rightDisplay = PhocaDownloadHelper::getUserRight('accessuserid', $this->file[0]->cataccessuserid, $this->file[0]->cataccess, $this->tmpl['user']->authorisedLevels(), $this->tmpl['user']->get('id', 0), 0);
}
// - - - - - - - - - - - - - - - - - - - - - -
...
if ($rightDisplay == 1) {
	...
	if ($this->tmpl['filetype'] == 'mp3') {
		...
		flowplayer("player", "<?php echo $this->tmpl['playerpath']; ?>flowplayer-<?php echo $versionFLP ?>.swf",
		{ 
			plugins: { 
				controls: { 
					fullscreen: false, 
		...
		flowplayer("player", "<?php echo $this->tmpl['playerpath']; ?>flowplayer-<?php echo $versionFLP ?>.swf");</script><?php
	}
	?></div></div>
