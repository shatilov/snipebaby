...
class PhocaDownloadViewRatingFileA extends JView
{
	...
	function display($tpl = null){
		...
		$ratingVote 	= JRequest::getVar( 'ratingVote', 0, 'post', 'int'  );
		$ratingId 		= JRequest::getVar( 'ratingId', 0, 'post', 'int'  );// ID of File
		$format 		= JRequest::getVar( 'format', '', 'post', 'string'  );
		$task 			= JRequest::getVar( 'task', '', 'get', 'string'  );
		$view 			= JRequest::getVar( 'view', '', 'get', 'string'  );
		$small			= JRequest::getVar( 'small', 1, 'get', 'string'  );//small or large rating icons
		...
		if ($task == 'refreshrate' && (int)$param['displayratingfile'] > 0) {			
			$ratingOutput 		= PhocaDownloadRateHelper::renderRateFile((int)$ratingId, 1, $small, true);// ID of File
			$response = array(
					'status' => '0',
			...
			$checkUserVote	= PhocaDownloadRateHelper::checkUserVoteFile( $post['fileid'], $post['userid'] );
