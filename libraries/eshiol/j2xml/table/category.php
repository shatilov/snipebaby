<?php
/**
 * @version		12.5.85 table/category.php
 * @package		J2XML
...
class eshTableCategory extends eshTable
{
	...
	*/
	...
	function toXML($mapKeysToText = false)
	{
		...
		$renamed = array(
			'created_user_id'=>'SELECT username created_by FROM '.$this->_db->nameQuote('#__users').' WHERE id = '.(int)$this->created_user_id,
			'modified_user_id'=>'SELECT username modified_by FROM '.$this->_db->nameQuote('#__users').' WHERE id = '.(int)$this->modified_user_id,
			);
