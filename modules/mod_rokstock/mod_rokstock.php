...
if ($save_cookie) {
    if (!isset($_COOKIE[$cookie_name])) setcookie($cookie_name, urlencode($stocklist), time() + 60 * 60 * 24 * $duration_cookie, '/'); else if (isset($_COOKIE[$cookie_name])) $stocklist = urldecode(JRequest::getVar($cookie_name, $default_cookie, 'COOKIE'));
}
