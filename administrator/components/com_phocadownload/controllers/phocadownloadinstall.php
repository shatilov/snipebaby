...
class PhocaDownloadCpControllerPhocaDownloadinstall extends PhocaDownloadCpController
{
	...
	function install() {		
		...
		$query.=' '.$db->nameQuote('access').' int(11) unsigned NOT NULL default \'0\','."\n";
		...
		*/
		...
		$query.=' '.$db->nameQuote('video_filename').' varchar(255) NOT NULL default \'\','."\n";
		...
		$query.=' '.$db->nameQuote('mirror1link').' varchar(255) NOT NULL default \'\','."\n";
		$query.=' '.$db->nameQuote('mirror1title').' varchar(255) NOT NULL default \'\','."\n";
		$query.=' '.$db->nameQuote('mirror1target').' varchar(10) NOT NULL default \'\','."\n";
		
		$query.=' '.$db->nameQuote('mirror2link').' varchar(255) NOT NULL default \'\','."\n";
		$query.=' '.$db->nameQuote('mirror2title').' varchar(255) NOT NULL default \'\','."\n";
		$query.=' '.$db->nameQuote('mirror2target').' varchar(10) NOT NULL default \'\','."\n";
		
		$query.=' '.$db->nameQuote('description').' text,'."\n";
		$query.=' '.$db->nameQuote('features').' text,'."\n";
		$query.=' '.$db->nameQuote('changelog').' text,'."\n";
		$query.=' '.$db->nameQuote('notes').' text,'."\n";
...
*/
		...
		// ------------------------------------------
		$query ='DROP TABLE IF EXISTS '.$db->nameQuote('#__phocadownload_file_votes').';'."\n";
		
		$db->setQuery( $query );
		if (!$result = $db->query()){$msgSQL .= $db->stderr() . '<br />';}
	
		$query =' CREATE TABLE '.$db->nameQuote('#__phocadownload_file_votes').' ('."\n";
		$query.=' '.$db->nameQuote('id').' int(11) NOT NULL auto_increment,'."\n";
		$query.=' '.$db->nameQuote('fileid').' int(11) NOT NULL default 0,'."\n";
		$query.=' '.$db->nameQuote('userid').' int(11) NOT NULL default 0,'."\n";
		$query.=' '.$db->nameQuote('date').' datetime NOT NULL default \'0000-00-00 00:00:00\','."\n";
		...
		// ------------------------------------------
		$query ='DROP TABLE IF EXISTS '.$db->nameQuote('#__phocadownload_file_votes_statistics').';'."\n";
		
		$db->setQuery( $query );
		if (!$result = $db->query()){$msgSQL .= $db->stderr() . '<br />';}

		$query =' CREATE TABLE '.$db->nameQuote('#__phocadownload_file_votes_statistics').' ('."\n";
		$query.=' '.$db->nameQuote('id').' int(11) NOT NULL auto_increment,'."\n";
		$query.=' '.$db->nameQuote('fileid').' int(11) NOT NULL default 0,'."\n";
		$query.=' '.$db->nameQuote('count').' int(11) NOT NULL default \'0\','."\n";
		...
		$query =' DROP TABLE IF EXISTS '.$db->nameQuote('#__phocadownload_tags').' ;';
		$db->setQuery( $query );
		if (!$result = $db->query()){$msgSQL .= $db->stderr() . '<br />';}
		
		$query ='CREATE TABLE '.$db->nameQuote('#__phocadownload_tags').' ('."\n";
		$query.=' '.$db->nameQuote('id').' int(11) NOT NULL auto_increment,'."\n";
		$query.=' '.$db->nameQuote('title').' varchar(255) NOT NULL default \'\','."\n";
		$query.=' '.$db->nameQuote('alias').' varchar(255) NOT NULL default \'\','."\n";
		...
		$query =' DROP TABLE IF EXISTS '.$db->nameQuote('#__phocadownload_tags_ref').' ;';
		$db->setQuery( $query );
		if (!$result = $db->query()){$msgSQL .= $db->stderr() . '<br />';}
		
		$query ='CREATE TABLE '.$db->nameQuote('#__phocadownload_tags_ref').' ('."\n";
		$query.=' '.$db->nameQuote('id').' SERIAL,'."\n";
		$query.=' '.$db->nameQuote('fileid').' int(11) NOT NULL default 0,'."\n";
		$query.=' '.$db->nameQuote('tagid').' int(11) NOT NULL default 0,'."\n";
		$query.=' PRIMARY KEY  ('.$db->nameQuote('id').'),'."\n";
		$query.=' UNIQUE KEY '.$db->nameQuote('i_fileid').' ('.$db->nameQuote('fileid').','.$db->nameQuote('tagid').')'."\n";
		$query.=') default CHARSET=utf8;'."\n";
		...
		$query =' DROP TABLE IF EXISTS '.$db->nameQuote('#__phocadownload_layout').' ;';
		$db->setQuery( $query );
		if (!$result = $db->query()){$msgSQL .= $db->stderr() . '<br />';}
		
		$query ='CREATE TABLE '.$db->nameQuote('#__phocadownload_layout').' ('."\n";
		$query.=' '.$db->nameQuote('id').' int(11) NOT NULL auto_increment,'."\n";
		$query.=' '.$db->nameQuote('categories').' text,'."\n";
		$query.=' '.$db->nameQuote('category').' text,'."\n";
		$query.=' '.$db->nameQuote('file').' text,'."\n";
		$query.=' '.$db->nameQuote('checked_out').' int(11) unsigned NOT NULL default \'0\','."\n";
		$query.=' '.$db->nameQuote('checked_out_time').' datetime NOT NULL default \'0000-00-00 00:00:00\','."\n";
		$query.=' '.$db->nameQuote('params').' text,'."\n";
		$query.=' PRIMARY KEY  ('.$db->nameQuote('id').')'."\n";
		...
		$query ='INSERT INTO '.$db->nameQuote('#__phocadownload_layout').' ('."\n";
		$query.=' '.$db->nameQuote('id').' ,'."\n";
		$query.=' '.$db->nameQuote('categories').' ,'."\n";
		$query.=' '.$db->nameQuote('category').' ,'."\n";
		$query.=' '.$db->nameQuote('file').' ,'."\n";
		$query.=' '.$db->nameQuote('checked_out').' ,'."\n";
		$query.=' '.$db->nameQuote('checked_out_time').' ,'."\n";
		$query.=' '.$db->nameQuote('params').' )'."\n";
		$query.=' VALUES ('."\n";
		$query.=' NULL,'."\n";
...
<div class="pd-cb"></div>
	...
	function upgrade()
	{
		...
		*/
		...
		$query ='CREATE TABLE IF NOT EXISTS '.$db->nameQuote('#__phocadownload_tags').' ('."\n";
		$query.=' '.$db->nameQuote('id').' int(11) NOT NULL auto_increment,'."\n";
		$query.=' '.$db->nameQuote('title').' varchar(255) NOT NULL default \'\','."\n";
		$query.=' '.$db->nameQuote('alias').' varchar(255) NOT NULL default \'\','."\n";
		$query.=' '.$db->nameQuote('link_cat').' int(11) NOT NULL default 0,'."\n";
		...
		$query ='CREATE TABLE IF NOT EXISTS'.$db->nameQuote('#__phocadownload_tags_ref').' ('."\n";
		$query.=' '.$db->nameQuote('id').' SERIAL,'."\n";
		$query.=' '.$db->nameQuote('fileid').' int(11) NOT NULL default 0,'."\n";
		$query.=' '.$db->nameQuote('tagid').' int(11) NOT NULL default 0,'."\n";
		$query.=' PRIMARY KEY  ('.$db->nameQuote('id').'),'."\n";
		...
		$query ='CREATE TABLE IF NOT EXISTS '.$db->nameQuote('#__phocadownload_layout').' ('."\n";
		$query.=' '.$db->nameQuote('id').' int(11) NOT NULL auto_increment,'."\n";
		$query.=' '.$db->nameQuote('categories').' text,'."\n";
		$query.=' '.$db->nameQuote('category').' text,'."\n";
		$query.=' '.$db->nameQuote('file').' text,'."\n";
	...
	*/
		...
		*/
		...
		$query =' SELECT * FROM '.$db->nameQuote('#__phocadownload_tags').' LIMIT 1;';
		$db->setQuery( $query );
		$result = $db->loadResult();
		if ($db->getErrorNum()) {
			$msgSQL .= $db->getErrorMsg(). '<br />';
		...
		$query =' SELECT * FROM '.$db->nameQuote('#__phocadownload_layout').' LIMIT 1;';
		$db->setQuery( $query );
